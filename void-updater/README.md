...
- `sh`, `awk`, `sed`, `test` and `uname`, required — base tools from
  any standard Void install, used to run and parse the xbps check and the
  reboot probe, and to run the upgrade.
- `sudo` or `doas`, required for the update to gain root. The **Elevation**
  setting picks one: Auto (default) uses `sudo` when present, else `doas`.
...
Everything except `xbps-install`, `sh`, `sudo`/`doas`, `awk`, `sed`, `test`
and `uname` is optional. A missing optional tool is skipped, not treated as an
error.
...
| `elevation` | `select` | `auto` | How to gain root for the update: `sudo`, `doas`, or none. Auto prefers `sudo`, else `doas`. |
...
- **Commands spawned.** `xbps-install -Mun` for the check; `flatpak list` /
  ...
  `sudo xbps-install -Su` (or `-Suy` with **Answer yes**; `doas` when that's
  the configured elevation), optionally
...
- **Privileges.** The plugin never elevates anything itself. Updates run via
  the configured elevation (`sudo xbps-install` or `doas xbps-install`, Auto
  picks whatever is installed) in the terminal, where it prompts normally.
